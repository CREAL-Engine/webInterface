...
#I forgot what this is right here
$|++;
...
$xmlIDData = `curl -s "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=aging+human&retmax=100000"`;
...
for ($i = 3; $i < @pubMedIdList; $i++)
{
	...
	#~ exit 0;
				...
				$mainHashObject{'cleanedAbstractData'} = $abstractData;
				$mainHashObject{'cleanedAbstractData'} =~ s/$qrstring//gi;
